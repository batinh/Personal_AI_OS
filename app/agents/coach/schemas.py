...
class WorkoutDay(BaseModel):
    ...
    workout_type: Literal["Easy", "Tempo", "Interval", "LongRun", "Recovery", "Rest", "CrossTraining"]
    title: str = Field(description="Vietnamese workout title, e.g. 'Chạy nhẹ dưỡng sức'")
...
class WeeklyPlanResult(BaseModel):
    ...
    week_total_km: float
    training_rationale: str = Field(description="Vietnamese 3-4 sentence explanation of this week's logic")
    acwr_projection: float = Field(description="Projected ACWR after completing this week")
    days: List[WorkoutDay] = Field(description="Exactly 7 entries Mon-Sun")
    adaptations_made: List[str] = Field(description="What the AI adjusted vs ideal plan, Vietnamese")
