...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        f"[STARTUP] DB path     : {DB_PATH} (exists: {os.path.exists(DB_PATH)})"
    )
    logger.info(
        f"[STARTUP] Config path : {CONFIG_PATH} (exists: {os.path.exists(CONFIG_PATH)})"
    )
    ...
    if cfg:
        logger.info(
            f"[STARTUP] Config loaded. Model: {cfg.get('model_name', 'default')}"
        )
        apply_log_levels(cfg.get("log_levels", {}))
    else:
        logger.warning(
            "[STARTUP] Config is EMPTY — system will run with defaults. Set up via Admin UI."
        )
