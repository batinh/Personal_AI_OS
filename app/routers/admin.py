...
async def save_settings(
    ...
    config["email_config"]["smtp_server"] = config.get("email_config", {}).get(
        "smtp_server", "smtp.gmail.com"
    )
    config["email_config"]["smtp_port"] = config.get("email_config", {}).get(
        "smtp_port", 587
    )
...
async def catch_accidental_get_save(username: str = Depends(verify_admin)):
    ...
    """
    logger.info(
        f"[ADMIN] Caught accidental GET request to /admin/save from user '{username}'. Redirecting to home..."
    )
    return RedirectResponse(url="/admin", status_code=303)
