...
|----|------|-------|----------|----------|------------|--------|--------|--------|
| [ISS-022](#iss-022--plan-command-shows-daily-suggestion-instead-of-generating-plan) | bug | `/plan` shows daily suggestion instead of generating a plan | High | U | 2026-05-22 | 2026-05-22 | `70565ae` | `coach/agent.py`, `database.py` |
...
### ISS-022 — `/plan` shows daily suggestion instead of generating a plan

**Type:** bug · **Priority:** High · **Reporter:** U · **Date:** 2026-05-22
**Module:** `app/agents/coach/agent.py`, `app/core/database.py`
**Commit:** `70565ae`

**Symptom:**
User types `/plan` in Telegram. Coach responds with a daily training suggestion instead of generating or showing a weekly plan. User has no way to trigger plan creation manually.

**Root Causes (3 bugs):**
1. `/plan` handler called `get_upcoming_plans()` and if empty, fell into `else` → `compute_daily_suggestion()`. It never called `generate_weekly_plan()` on demand.
2. Pending plans (generated by Sunday scheduler but not yet `/accept`ed) were invisible to `/plan` — the handler only checked `training_plans`, not `weekly_plans`.
3. `get_upcoming_plans()` returned a Vietnamese string on empty (`"Chưa có giáo án..."`) which is truthy — so the `if upcoming:` guard fired, displayed the error string as if it were a plan, and never fell through to generate one.

**Fix:**
- `agent.py:/plan handler`: 3-step fallback — (1) show accepted plan from `training_plans`, (2) re-surface pending plan from `weekly_plans` with `/accept` prompt, (3) call `generate_weekly_plan()` on demand.
- `database.py:get_upcoming_plans`: return `""` (falsy) instead of Vietnamese error string when no rows found.
