...
class TestE2EStravaFullWorkflow(unittest.TestCase):
    ...
    def test_create_event_triggers_full_pipeline(
        ...
        with patch(
            "app.routers.webhooks.StravaClient", return_value=mock_strava_client
...
class TestE2ETimezoneUtils(unittest.TestCase):
    ...
    def test_morning_briefing_flow_uses_timezone_utils(self):
        """Proves timezone handling uses get_local_tz directly (TD-003 refactor)."""
        import inspect
        import app.agents.coach.flows.morning_briefing as mb
        import app.core.timezone_utils as tz_mod

        src = inspect.getsource(mb)
        tz_src = inspect.getsource(tz_mod)
        # After TD-003 refactor, get_local_tz is used directly in each flow file
        self.assertIn("get_local_tz", src)
        self.assertIn("get_local_tz", tz_src)
