...
class TestAcceptWeeklyPlan:
    ...
    def test_returns_confirmed_message_on_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            wf, "get_pending_weekly_plan", lambda uid, ws=None: plan_row
        )
...
class TestRejectWeeklyPlan:
    ...
    def test_returns_rejected_message(self, monkeypatch):
        ...
        plan_row = {
            "id": 1,
            "ai_output": plan.model_dump_json(),
            "week_start_date": "2030-06-02",
        }
        monkeypatch.setattr(
            wf, "get_pending_weekly_plan", lambda uid, ws=None: plan_row
        )
        ...
        conn.execute(
            "CREATE TABLE weekly_plans (id INTEGER PRIMARY KEY, user_id TEXT, week_start_date TEXT, status TEXT, ai_output TEXT)"
        )
