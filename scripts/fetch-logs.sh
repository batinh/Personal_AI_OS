...
if ! docker inspect "$CONTAINER" &>/dev/null; then
    echo -e "${RED}[ERROR]${NC} Container '$CONTAINER' not found." >&2
    echo "Available containers: $(docker ps --format '{{.Names}}' | tr '\n' ' ')" >&2
    exit 1
fi
...
build_grep_pattern() {
    ...
    if [[ -n "$LEVEL_FILTER" ]]; then
        # Convert comma-separated levels to regex alternation: ERROR,WARNING → ERROR|WARNING
        level_pat=$(echo "$LEVEL_FILTER" | tr ',' '|' | tr '[:lower:]' '[:upper:]')
    fi

    if [[ -n "$MODULE_FILTER" ]]; then
        # Match [MODULE] log tag (case-insensitive keyword)
